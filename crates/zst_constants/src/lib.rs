...
use syn::{parse_macro_input, DeriveInput, Ident, LitFloat, LitInt, LitStr};
...
pub fn as_constant(input: TokenStream) -> TokenStream {
    ...
    let maybe_output = match args.const_type.value().as_str() {
        "struct" => process_struct(args.const_value, struct_name),
...
fn process_struct(json_lit: LitStr, original_name: &Ident) -> Result<TokenStream2, syn::Error> {
    let json_str = json_lit.value();
    let _: Value = match serde_json::from_str(&json_str) {
        Ok(v) => v,
        Err(e) => {
            let error_msg = format!("Failed to parse JSON: {}", e);
            return Err(syn::Error::new_spanned(json_lit, error_msg));
        }
    };

    let constant_value = json_lit.to_token_stream();
...
fn process_string(string_lit: LitStr, original_name: &Ident) -> Result<TokenStream2, syn::Error> {
    let output = quote! {
        impl<'de> serde::Deserialize<'de> for #original_name {
            fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
            where
...
fn process_int(string_lit: LitStr, original_name: &Ident) -> Result<TokenStream2, syn::Error> {
...
fn process_number(string_lit: LitStr, original_name: &Ident) -> Result<TokenStream2, syn::Error> {
